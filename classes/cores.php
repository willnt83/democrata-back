...
class Cores{
    ...
    public function getCores($filters){
        $where = '';
        if(count($filters) > 0){
            $where = 'where ';
            $i = 0;
    ...
    public function createUpdateCor($request){
        try{
            // Validações
    ...
    public function deleteCor($filters){
        try{
            $sql = '
                delete from pcp_cores
                where id = :id
            ';
            $stmt = $this->pdo->prepare($sql);
