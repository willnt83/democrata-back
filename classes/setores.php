...
class Setores{
    ...
    public function getSetoresTitulo($filters){
        $sql = '
            SELECT s.id, s.nome
            FROM pcp_setores s
            WHERE s.ativo = "Y"
    ...
    public function getSetores($filters){
        $where = '';
        if(count($filters) > 0){
            $where = 'where ';
            $i = 0;
    ...
    public function createUpdateSetor($request){
        try{
            // Validações
    ...
    public function deleteSetor($filters){
        try{
            $sql = '
                delete from pcp_setores
                where id = :id
            ';
            $stmt = $this->pdo->prepare($sql);
            ...
            return json_encode(array(
                'success' => true,
                'msg' => 'Setor removido com sucesso.'
            ));
        }
